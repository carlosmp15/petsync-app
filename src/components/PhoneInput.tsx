...
const PhoneInput = React.forwardRef<HTMLInputElement, PhoneInputProps>(
  ({ className, onChange, onCountryChange, ...props }, ref) => {
    // Wrap InputComponent to forward the ref to the input element
...
const CountrySelect = ({ disabled, value: selectedCountry, options: countryList, onChange }: CountrySelectProps) => {
  // Add state to control the open state of the Popover
  const [open, setOpen] = React.useState(false)

  // Create a handler that will both change the country and close the popover
                ...
                {countryList.map(({ value, label }) =>
                  value ? (
                    <CountrySelectOption
                      ...
                      onChange={handleCountryChange} // Use our new handler
                    />
                  ) : null,
                )}
              </CommandGroup>
